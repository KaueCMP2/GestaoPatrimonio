...
VALUES (NEWID(),'Sem local', NULL, NULL, (select a.AreaID from area a where a.NomeArea = 'Sem área'));

INSERT INTO Endereco (EnderecoID, Logradouro, Numero, Complemento, CEP, BairroID) 
VALUES
(NEWID(), 'Rua sao pedro da sorte', 44, 'casa 1', '12345678', (
SELECT b.BairroID FROM Bairro b
WHERE b.NomeBairro = 'Centro'))
GO;

INSERT INTO Usuario (UsuarioID, NIF, Nome, RG, CPF, CarteiraTrabalho, Email, Senha, EnderecoID, CargoID, TipoUsuarioID)
VALUES
(NEWID(), '1234567', 'Admin',  '123456789', '12345678910', '12345678910', 'a@a.com', HASHBYTES('SHA2_256', 'admin'), 
(SELECT e.EnderecoID FROM Endereco e
WHERE e.Logradouro = 'Rua sao pedro da sorte' 
AND e.Numero = 44),
(SELECT c.CargoID FROM Cargo c
WHERE c.NomeCargo = 'Diretor'),
(SELECT t.TipoUsuarioID FROM TipoUsuario t
WHERE T.NomeTipo = 'Coordenador'))	
GO